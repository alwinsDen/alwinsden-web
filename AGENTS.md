...
Package-specific commands, constraints, and gotchas live in each area's own `AGENTS.md` — `packages/ai-keyboard`, `packages/alwinsden-cloudflare`, `service/`. All of them plus this file are loaded in every session via `instructions` in `opencode.json`, so treat them as one combined rule set.
...
- `pnpm dev:cloudflare` — main site and article development server
- `pnpm build:cloudflare` — builds the unified UI packages and the web app
...
- Deploys (need `wrangler login` or `CLOUDFLARE_API_TOKEN`): `pnpm --filter alwinsden-cloudflare run deploy`
...
- `node-linker=hoisted` in `.npmrc` is deliberate (Vite + Metro resolve from root `node_modules`). Don't remove or switch to isolated.
- Build output goes to the **repo root** `dist/cloudflare`, not into the packages (set in `vite.config.ts` and `react-router.config.ts`).
