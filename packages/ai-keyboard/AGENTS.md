...
- **Stale `ios/Pods`**: iOS build failing with `<header> file not found` from a pod's own C++ source (e.g. reanimated's `'reanimated/CSS/utils/algorithms.h' file not found`) usually means this — CocoaPods' incremental install never rebuilds `Pods/` after dependency paths move under an unchanged `Podfile.lock`. Use the `build-run-issue-fix` skill to diagnose and fix (short version: `rm -rf packages/ai-keyboard/ios/Pods packages/ai-keyboard/ios/build`, then re-run `pnpm dev:aikb:ios`).
...
- The app is **dark-mode only** — no light/dark switching. Dark is hardcoded at every layer: `DarkTheme` + `MD3DarkTheme` in `_layout.tsx`, `Colors` in `constants/theme.ts` is the dark palette only, `hooks/use-theme.ts` just returns `Colors`, and `userInterfaceStyle: "dark"` in `app.json` forces native chrome dark. The `hooks/use-color-scheme.*` files were deleted — don't reintroduce `useColorScheme()` for theming. Note `userInterfaceStyle` is baked in at prebuild, so a `pnpm dev:aikb:*` re-run is needed for that change to take effect on device.
